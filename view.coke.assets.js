...
function Asset(record) {
	...
	this.assetGroup = record.field(this.ASSETGROUP);
}

Asset.prototype.LOCATION = 102; // LAL was 15 - switched to Placement
Asset.prototype.ASSETGROUP = 109;
...
Asset.prototype.toBox = function(separator) {
    ...
    var spdate, epdate; // set wide net for project times if none set (~2000 - 2020)
    ...
    cleanup2 = this.shortName.indexOf('"')
    if(cleanup >= 0) {
		this.projectName = string_replace(this.projectName,'"','');
    }
	if(cleanup2 >= 0) {
		this.shortName = string_replace(this.projectName,'"','');
    }
    
    total_assets.push(this.recordId)
    
    if(this.shortName != '') {
		labelName = this.shortName;
	} else {
		labelName = this.projectId + ': ' + this.projectName;
	}
...
AssetHeader = function(assetGroupings, header, assetGroup) {
...
AssetHeader.prototype.toXMLString = function() {
	var flag = $.getUrlVar('pageID'); // used to determine order of assets
		if(this.header == 'All Promo Landing Promo') this.header = '*All Promo Landing Promo';
		if(this.header == 'Limited Time Promo Landing Promo') this.header = '*Limited Time Promo Landing Promo';
		if(this.header == 'AUX' || this.header == 'Newsletter' || this.header == 'SMS') this.header = this.header+' (Outbound)';
